...
if [ -f "${PRIVATE_KEY}" ] && [ -f "${PUBLIC_KEY}" ]; then
    log "Existing GPG key pair found. Skipping key generation."
else
    log "No existing GPG key pair found. Generating a new one..."
...
EOF
    ...
    log "GPG key pair generated successfully."
fi
...
if ! "${SCRIPT_DIR}/validate-pgp.sh"; then
